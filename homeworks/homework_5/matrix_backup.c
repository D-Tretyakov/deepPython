...
static int cast_to_c(PyObject*, long**, int*, int*);
static int cast_to_py(PyObject**, long*, int, int);
...
static int cast_to_c(PyObject *pyArray, long **cArray, int *i_size, int *j_size) 
{   
    ...
    *cArray = (long *) realloc(*cArray, sizeof(long) * (*i_size) * (*j_size));
    if (*cArray == NULL) {
...
static int cast_to_py(PyObject **pyArray, long *cArray, int i_size, int j_size) 
{
    ...
    for (int i = 0; i < i_size; i++) {
        ...
        for (int j = 0; j < j_size; j++) {
            PyObject *value = Py_BuildValue("i", cArray[i * (j_size) + j]);
...
static void dot_product(long *a1, long *a2, long *res, 
...
Matrix_mul(PyObject* obj1, PyObject* obj2) 
{
    ...
    if (PyLong_Check(obj1)) {
        ...
        long value = PyLong_AsLong(obj1);
        long* new_cvalues = (long*)malloc(sizeof(long)*self->i_size * self->j_size);
        for(int i = 0; i < self->i_size * self->j_size; i++) {
    ...
    if (Py_TYPE(obj1) == Py_TYPE(obj2)) { // matrices multiplication
        ...
        long* new_cvalues = (long*)malloc(sizeof(long)*m1->i_size * m2->j_size);
...
Matrix_add(PyObject* obj1, PyObject* obj2) 
{
    ...
    if (Py_TYPE(obj1) == Py_TYPE(obj2)) { // matrices multiplication
        ...
        long* new_cvalues = (long*)malloc(sizeof(long)*m1->i_size * m2->i_size);
